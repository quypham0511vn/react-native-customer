...
const Notify = observer(() => {
    ...
    }, [apiServices.notification, lastId, pageSize]);
    ...
    const renderEmptyData = useMemo(() => {
        return (
            <Text style={styles.textEmpty}>{Languages.notify.noNotify}</Text>
        );
    }, []);
    ...
    const renderEmpty = useMemo(() => {
        return (notifications?.length === 0 && !canLoadMore) ? renderEmptyData : null;
    }, [canLoadMore, notifications?.length, renderEmptyData]);
    ...
    const renderList = useMemo(() => {
        return <MyFlatList
            ...
            {... { keyExtractor }}
        />;
    }, [notifications, renderListNotify, renderFooter, renderEmpty, isRefreshing, onRefresh, onEndReached, onMomentumScrollBegin, keyExtractor]);
